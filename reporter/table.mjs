...
export function benchmark_error(
  ...
  return `${n.padEnd(size, ' ')}${kleur.red(colors, 'error')}: ${e.message}${
...
export function benchmark(
  ...
  return (
    ...
    (!avg
      ? ''
      : `${kleur.yellow(colors, duration(b.avg))}/iter`.padStart(
          14 + 10 * colors,
          ' ',
        )) +
    (!min_max
      ? ''
      : `(${kleur.cyan(colors, duration(b.min))} … ${kleur.magenta(
          colors,
          duration(b.max),
...
export function summary(benchmarks, { colors = true } = {}) {
  ...
  return `${
    kleur.bold(colors, 'summary') +
    (null == baseline.group || baseline.group.startsWith?.('$mitata_group')
      ? ''
      : kleur.gray(colors, ` for ${baseline.group}`))
  }\n  ${kleur.bold(colors, kleur.cyan(colors, baseline.name))}${benchmarks
    .filter(b => b !== baseline)
    .map(b => {
      ...
      return `\n   ${kleur[1 > diff ? 'red' : 'green'](
        colors,
        1 <= diff ? diff : inv_diff,
      )}x ${1 > diff ? 'slower' : 'faster'} than ${kleur.bold(
        colors,
        kleur.cyan(colors, b.name),
      )}`;
    })
    .join('')}`;
}
